...
def plot_full_map(nn_model, data_set, gt, int_to_vector_dict, patch_size, plo=True):
    """
    Plots prediction map for the entire pixels and includes a legend for unique classes in ground truth (gt).
    ...
    legend : dict
        A dictionary containing class labels as keys and corresponding colors as values.
    """
    # Generate the grid of pixel indices
    rr, cc = np.meshgrid(np.arange(gt.shape[0]), np.arange(gt.shape[1]))
    all_pixel_indices = (rr.ravel(), cc.ravel())
    vector_2_label = one_hot_2_label(int_to_vector_dict)
    
    # Create patches for model input
    all_inputs, all_labels = create_patch(data_set, gt, all_pixel_indices, patch_size, int_to_vector_dict)
    
    # Expand input dimensions for 3D convolution if necessary
    if len(nn_model.input_shape) == 5:
        all_inputs = np.array([np.expand_dims(elm, -1) for elm in all_inputs])
        
    # Predict all labels using the trained model
    all_y_pred_vectors = nn_model.predict(all_inputs, batch_size=1)
    all_y_pred = np.zeros(all_y_pred_vectors.shape[0], dtype=int)
    
    # Convert the predicted one-hot vectors to integer labels
    ...
    # Generate a 2D prediction map for all pixels
    ...
    # Generate the legend based on unique values in ground truth (gt)
    ...
    # Prepare the legend dictionary
    # legend_classes = {class_label: colors[i] for i, class_label in enumerate(unique_classes)}
    
    if plo:
        # Plot the prediction map
        fig, ax = plt.subplots(figsize=(8, 6))
        cax = ax.imshow(gt_pred_all_map, cmap=cmap)  # Use same colormap here
        ...
        ax.legend(handles=handles, labels=labels, loc='upper right', title="Classes")
        ax.set_title("Prediction map for all data", fontweight="bold", fontsize=14)
        plt.show()
    
    return gt_pred_all_map, unique_classes
